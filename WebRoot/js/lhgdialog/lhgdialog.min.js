...
lhgdialog.select = function(yes, parent )
{
	...
	return lhgdialog({
		...
		content: [
			'<div>',
                '<select name="audit"><option value="同意">同意</option><option value="不同意">不同意</option></select>'
			,
			'</div>'
			].join(''),
		init: function(){
			input = this.DOM.content[0].getElementsByTagName('select')[0];
			input.select();
			input.focus();
		},
		ok: function(here){
