...
<%
	...
	String add_uri="backstage/content/add/"+parent_id+"/"+parent_name;
...
<% if(!MCheck.isNull(lst)){for(MRecord row:lst){%>
	<%=T.tr_align("center") %>
		...
		<%=T.td("left",row.getStr("category_name"),"10%") %>
		<%=T.td("13%",row.getStr("market_price")+"-"+row.getStr("preferential_price")) %>
		<%=T.td("20%",row.getStr("starttime")+"--"+row.getStr("endtime")) %>
		<%=T.td("8%",row.getStr("is_recommend").equals("false")?"否":"是") %>
		<%=T.td("17%",row.getStr("pubdate")) %>
		<%=T.td() %>
    		<%=T.a_edit_base("backstage/content/edit/"+row.getStr("id")+"/"+parent_id+"/"+parent_name) %>
    		<%=T.a_del_base("backstage/content/del/"+row.getStr("id")+"/"+parent_id+"/"+parent_name) %>
    	<%=T.td_close() %>
	<%=T.tr_close() %>
<%	}}else{ %>
	<%=T.tr("center") %>
